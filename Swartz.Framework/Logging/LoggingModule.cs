...
namespace Swartz.Logging
{
    public class LoggingModule : Module
    {
        ...
        {
            builder.RegisterType<SwartzLog4NetFactory>().As<ILoggerFactory>().InstancePerLifetimeScope();
        ...
        private IEnumerable<Action<IComponentContext, object>> BuildLoggerInjectors(Type componentType)
        {
            ...
            var loggerProperties = componentType.GetProperties(BindingFlags.SetProperty | BindingFlags.Public |
                                                               BindingFlags.Instance).Select(p => new
                                                               {
                                                                   PropertyInfo = p,
                                                                   p.PropertyType,
