...
{
    public class DefaultSwartzHostContainer : IWebHostContainer, IDependencyResolver
    {
        private readonly IContainer _container;

        public DefaultSwartzHostContainer(IContainer container)
        {
        ...
        {
            return
                Resolve<IEnumerable>(typeof(IEnumerable<>).MakeGenericType(serviceType), Enumerable.Empty<object>())
                    .Cast<object>();
        }

        TService IWebHostContainer.Resolve<TService>()
        {
            // Resolve service, or null
