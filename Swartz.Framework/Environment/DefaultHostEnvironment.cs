...
namespace Swartz.Environment
{
    public class DefaultHostEnvironment : HostEnvironment
    {
        ...
        public override void RestartAppDomain()
        {
            ...
            if (!success)
            {
                throw new SwartzException(
                    "We needs to be restarted due to a configuration change, but was unable to do so.\r\n" +
                    ...
                    $"- give the application write access to the '{HostRestartPath}' folder, or\r\n" +
                    $"- give the application write access to the '{WebConfigPath}' file.");
            }
