    ...
    public class AppDomainAssemblyNameResolver : IAssemblyNameResolver
    {
        ...
        public string Resolve(string shortName)
        {
            return AppDomain.CurrentDomain
                ...
                .Where(
                    a =>
                        StringComparer.OrdinalIgnoreCase.Equals(shortName,
                            AssemblyLoaderExtensions.ExtractAssemblyShortName(a.FullName)))
                .Select(a => a.FullName)
                .SingleOrDefault();
        }
    }

    public class SwartzFrameworkAssemblyNameResolver : IAssemblyNameResolver
    {
        private readonly ICacheManager _cacheManager;

        public SwartzFrameworkAssemblyNameResolver(ICacheManager cacheManager)
        {
        ...
        public string Resolve(string shortName)
        {
            // A few common .net framework assemblies are referenced by the Swartz.Framework assembly.
            // Look into those to see if we can find the assembly we are looking for.
            var swartzFrameworkReferences = _cacheManager.Get(typeof(IAssemblyLoader), true, ctx =>
                ctx.Key.Assembly
                    .GetReferencedAssemblies()
                    .GroupBy(n => AssemblyLoaderExtensions.ExtractAssemblyShortName(n.FullName),
                        StringComparer.OrdinalIgnoreCase)
                    .ToDictionary(n => n.Key /*short assembly name*/, g => g.OrderBy(n => n.Version).Last()
                        /* highest assembly version */, StringComparer.OrdinalIgnoreCase));

            AssemblyName assemblyName;
            if (swartzFrameworkReferences.TryGetValue(shortName, out assemblyName))
            {
        ...
        public string Resolve(string shortName)
        {
            ...
            var lookup = _cacheManager.Get("---", ctx =>
                GetGacListForDotNet40()
                    ...
                    .ToDictionary(AssemblyLoaderExtensions.ExtractAssemblyShortName,
                        StringComparer.OrdinalIgnoreCase));
