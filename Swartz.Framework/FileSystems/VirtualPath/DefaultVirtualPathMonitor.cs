...
namespace Swartz.FileSystems.VirtualPath
{
    public class DefaultVirtualPathMonitor : IVirtualPathMonitor
    {
        ...
        private void BindSignal(string virtualPath, CacheItemRemovedCallback callback)
        {
            ...
            var cacheDependency = HostingEnvironment.VirtualPathProvider.GetCacheDependency(virtualPath,
                new[] {virtualPath}, _clock.UtcNow);

            Logger.Debug("Monitoring virtual path \"{0}\"", virtualPath);

            HostingEnvironment.Cache.Add(key, virtualPath, cacheDependency, Cache.NoAbsoluteExpiration,
