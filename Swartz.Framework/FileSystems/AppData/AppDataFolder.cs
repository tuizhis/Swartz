    ...
    public class AppDataFolder : IAppDataFolder
    {
        ...
        private void MakeDestinationFileNameAvailable(string destinationFileName)
        {
            ...
            catch (Exception ex)
            {
                ...
                throw new SwartzCoreException(
                    $"Unable to make room for file \"{destinationFileName}\" in \"App_Data\" folder", ex);
            }
        }
        ...
        {
            return PathValidation.ValidatePath(RootFolder,
                Path.Combine(RootFolder, Path.Combine(paths)).Replace('/', Path.DirectorySeparatorChar));
        }
    }
}